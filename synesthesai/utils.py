...
def pull_keys_to_top_level(data, keys):
    ...
    def traverse_dict(d):
        for key, value in list(
            d.items()
        ):  # We use list to avoid RuntimeError due to size change during iteration
            if key in keys:
...
def filter_values(list_to_filter, valid_values):
    remaining = [
        value.lower() for value in list_to_filter if value.lower() in valid_values
    ]
    if not remaining:
